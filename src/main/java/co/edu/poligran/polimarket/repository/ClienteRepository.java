...
public interface ClienteRepository extends JpaRepository<Cliente, Long>, JpaSpecificationExecutor<Cliente> {
    ...
    @Query(
        value = "select cliente from Cliente cliente left join fetch cliente.vendedor",
        countQuery = "select count(cliente) from Cliente cliente"
    )
    Page<Cliente> findAllWithToOneRelationships(Pageable pageable);

    @Query("select cliente from Cliente cliente left join fetch cliente.vendedor")
    List<Cliente> findAllWithToOneRelationships();

    @Query("select cliente from Cliente cliente left join fetch cliente.vendedor where cliente.id =:id")
