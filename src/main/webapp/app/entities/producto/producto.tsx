  ...
  return (
    <div>
      <h2 id="producto-heading" data-cy="ProductoHeading">
        <Translate contentKey="polimarketApp.producto.home.title">Productos</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="polimarketApp.producto.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/producto/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="polimarketApp.producto.home.createLabel">Create new Producto</Translate>
          </Link>
        </div>
      </h2>
        ...
        {productoList && productoList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="polimarketApp.producto.id">ID</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
                ...
                <th className="hand" onClick={sort('cantidadDisponible')}>
                  <Translate contentKey="polimarketApp.producto.cantidadDisponible">Cantidad Disponible</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('cantidadDisponible')} />
                </th>
                <th>
                  <Translate contentKey="polimarketApp.producto.proveedor">Proveedor</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th>
                  <Translate contentKey="polimarketApp.producto.tarea">Tarea</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {productoList.map((producto, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{producto.proveedor ? <Link to={`/proveedor/${producto.proveedor.id}`}>{producto.proveedor.nombre}</Link> : ''}</td>
                  <td>{producto.tarea ? <Link to={`/tarea/${producto.tarea.id}`}>{producto.tarea.descripcion}</Link> : ''}</td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="polimarketApp.producto.home.notFound">No Productos found</Translate>
            </div>
          )
        )}
      </div>
